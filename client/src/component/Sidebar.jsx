...
bg-white/80 dark:bg-gradient-to-b dark:from-[#242124]/80 dark:to-[#000000]/70
backdrop-blur-3xl border-r border-[#80609f]/30
transition-all duration-500 max-md:absolute left-0 z-[1] ${!isMenuOpen && 'max-md:-translate-x-full'} `}>
     ...
     <button className='flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6] text-sm rounded-md cursor-pointer'>
      ...
      {/* cummunity Image */}
      <div onClick={()=> {navigate('/community'), setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        <img src={assets.gallery_icon } className='w-4.5 not-dark:invert' alt=''/>
        <p>Community image</p>
      </div>

      {/* Credit purchases option */}
      <div onClick={()=> {navigate('/credits'), setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
      ...
      {/* dark mode toggle */}
      <div  className='flex items-center justify-between gap-2 p-3 mt-3 border border-gray-300 dark:border-white/15 rounded-md '>
 ...
 {/* logine page--------- */}
      ...
      <img onClick={()=>setIsMenuOpen(false)} src={assets.close_icon } className='absolute top-3 right-3 w-3 h-5 cursor-pointer md:hidden not-dark:invert' alt=''/>
